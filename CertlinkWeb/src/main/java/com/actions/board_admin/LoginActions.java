...
public class LoginActions {
	...
	public void DiplomateLogin(WebDriver driver, String username, String password, String scenario) {
		...
		if (scenario.equalsIgnoreCase("valid"))
			Assert.assertTrue(ca.isElementPresent(driver, dd.DDashboardUI), "FAIL: Login failed");
		if (scenario.equalsIgnoreCase("Invalid"))
			Assert.assertTrue(ca.isElementPresent(driver, login1.LoginButtonUI), "FAIL: Login failed");
	}

	public void DiplomateLogout(WebDriver driver, WebDriverWait wait) {
		ca.clickOnElement(driver, dd.DLogoutUI);
		wait.until(ExpectedConditions.visibilityOfElementLocated(dd.DSignOutButtonUI));
		ca.clickOnElement(driver, dd.DSignOutButtonUI);
		try {
			wait.until(ExpectedConditions.visibilityOfElementLocated(login1.LoginButtonUI));
			Assert.assertTrue(ca.isElementPresent(driver, login1.LoginButtonUI), "FAIL: Diplomate user unable to logout.");
		}
		catch(Exception e) {
